...
See `real_w4a4_dispatch/real_e2e_summary.json`: REAL_Q11 (target 224 real
QuaRot linears + draft real projections + real AR head, KV fp16) vs the fake
counterpart on identical prompts — recipe differences (per-channel sym absmax/7
+ per-token sym vs RTN+MSE-clip + per-token asym) produce the expected
magnitude-level agreement (both collapse the draft; numbers in the JSON), and
Gate-B-level kernel checks match fp emulation at rel 1.8e-2. Real-kernel
dispatch is proven per-module (forward counters + module types; no fp16
fallback on any quantized path).
...
shape sensitivity exists** — see `verifier_consistency/target_logit_consistency.csv`
(top-1 agreement < 1 between paths); consequently ea-vs-naive exact-match is
