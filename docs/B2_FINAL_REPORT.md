...
recurrent levels); Q11 coverage: projection_first n=1119 (=cycles), recurrent
4476=4×1119, each AR linear 5595=5×1119, all fake-quant counters live
(`coverage__quant.csv`).
...
|---|---:|---|---|
| rotation control FP02 vs stock | −0.004 | [−0.051, +0.032] | equivalent at ε=0.05* |
...
positions: 456 within noise; 165 overlap decreases; 62 increases classified as
**distribution flattening** (entropy↑, margin↓); 53 **incidental rank flips**;
32 alignment-like increases — and since PPL degrades +3.68 and margins shrink
...
W4A4 costs −1.4..−2.1. **projection_first is the single most damaging component**
(−2.00 alone ≈ both −2.14 ≈ full −2.14).

## 15. Does recurrent-projection error accumulate with depth? (fig4)

YES — and the split cleanly separates the two failure modes:
- recurrent-only quant: α₁=0.886 (first step intact) → α₂=0.077 → α₄≈0 —
  textbook depth-accumulating divergence;
- first-only quant: α₁=0.25 (initialization destroyed) then STABLE ≈0.3 at
  deeper depths (recurrent steps are clean).
...
variance. Local increases (12% of fixed positions) are flattening (8%) or
incidental rank flips (7%) under a degrading target (PPL +3.68, flips 17%),
i.e., degradation-phenomena — not benign alignment.
...
- Draft-quant results here are fake-quant (SpinQuant quantizers) with fp16
  ...
  the prior study (B2 = 3.815 ≈ A = 3.814, `runs/rotstudy_realint4_*`).
...
- n=20 MT-bench prompts; effects reported all have |Δ| ≥ 0.18 with dz ≥ 0.9,
  far outside the pre-registered ε.
