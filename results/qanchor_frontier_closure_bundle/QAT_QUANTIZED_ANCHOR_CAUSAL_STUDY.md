...
Status: COMPLETE, **superseded in part by the frontier closure audit**
(2026-08-16, `docs/QAT_QUANTIZED_ANCHOR_FRONTIER_CLOSURE.md`,
branch `exp/eagle1-qanchor-frontier-closure-audit`).

> **SUPERSEDED CLAIMS (closure verdict B + D).** The "frontier
> dominance" conclusion below was PROVISIONAL: its Pareto analysis
> omitted the tuned low-LR plain-QAT points (conv/hybrid @1e-6 reach
> mean4 3.59-3.62), against which the best hard-budget model is
> statistically indistinguishable on acceptance length (10k paired
> bootstrap + Holm: 0 wins / 0 losses; the conv b2% finalist loses
> gsm8k). The supported claim is EFFICIENCY dominance: same
> acceptance length with H_Q_controlled ~3e-5 (~700x lower) and
> D_Q 3.5x lower.
> **MECHANISM CORRECTION (W-B).** The "1-2% sparse utility-selected
> flips" story is falsified: 97.24% of the best HB model's changed
> codes are W_first h-half (uncontrolled by the budget; 20.3% flip
> rate there), only 7,276 codes (0.2% of flips) are in the budgeted
> folds, and deployment-code counterfactuals show W_first drift
> carries 75-94% of the gain (controlled flips alone: 28%). The
> budget works by channeling adaptation into the coupled first-fold
> basis, not by sparse selective flips.
> **H_Q accounting**: "H_Q 1.30%" below is H_Q_all; the budget label
> "b0.5%" refers to CONTROLLED sites only.
> **Parity wording**: "byte/value-equal" is corrected to value-exact
> in both modes; the identity-mode W_first tensor differs in exactly
> 37 fp16 signed-zero byte encodings (logits identical).
> **LoRA wording**: the Gate-C conclusion applies only to the tested
> adapter family (r {4,8,16} @1e-4, r8 @1e-3, 9 fold sites, 3000
> steps); "expressivity outside the cells is not the mechanism" is
> narrowed accordingly.

This document is the preregistered contract;
